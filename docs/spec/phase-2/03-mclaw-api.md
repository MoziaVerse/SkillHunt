...
// POST /api/install-tokens 201
{
  ...
  "maxUses": 1,
  "installCommand": "npx skills add http://localhost:3333/.well-known/agent-skills/i/Hk9q...x4 --skill mozia/foo --agent claude-code -y"
}
```
...
- capability URL 路径里的 owner/slug 与 grant 的 skill 不一致 → 404
...
# 拉 SKILL.md（应 200）
curl -fsS http://localhost:3333/.well-known/agent-skills/i/$TOK/mozia/foo/SKILL.md

# 第二次（应 404）
curl -sS -o /dev/null -w '%{http_code}\n' \
  http://localhost:3333/.well-known/agent-skills/i/$TOK/mozia/foo/SKILL.md
# 期望: 404
```
