...
# 6) 灌种子数据（4 referenced + 5 owned，含 1 internal）
pnpm seed:all
```
...
`apps/api/src/routes/api.test.ts` 和 `wellknown.test.ts` 在 `beforeEach` / `afterAll` 里会 `TRUNCATE skillhub.skills RESTART IDENTITY CASCADE`。**跑完 `pnpm test` 后种子数据会被清空**。要回到可用状态：

```bash
pnpm seed:all
```

如果 smoke 突然全挂、`/api/skills` 返 0 条，先跑 seed 再说。
...
| commit 前 | 上面两条 + `pnpm test` + `pnpm seed:all` |
| PR 前 / 怀疑 e2e 坏了 | 全套 + `bash scripts/smoke.sh`（要求 api+web 在跑）|
...
| `/api/skills` 返 0 | 跑过 `pnpm test` 把表清了，重新 `pnpm seed:all` |
